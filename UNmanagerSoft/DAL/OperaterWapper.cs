...
namespace UNmanagerSoft.DAL
{
    class OperaterWapper
    {
        ...
        public DataTable operaterSelect(string name)
        {
            string strConOffice = off.readConnOffice();
            OleDbConnection conn = new OleDbConnection(strConOffice);
            conn.Open();
            string sqlStr = "select * from 操作员表 where 名字 = " + "'" + name + "'";
            OleDbDataAdapter adapter = new OleDbDataAdapter(sqlStr, conn);
            DataTable dataset = new DataTable();
            adapter.Fill(dataset);
            conn.Close();
        ...
        public int operaterInsert(string name, string dateTime)
        {
            string strConOffice = off.readConnOffice();
            OleDbConnection conn = new OleDbConnection(strConOffice);
            conn.Open();
            string sqlStr = "insert into 操作员表 values('" + name + "','" +
                dateTime + "')";
        ...
        /// <returns>受影响的行数</returns>
        public int operaterDelete(string name)
        {
            string strConOffice = off.readConnOffice();
            OleDbConnection conn = new OleDbConnection(strConOffice);
            conn.Open();
            string sqlStr = "delete from 操作员表 where 名字 = '" + name + "'";
            OleDbCommand command = new OleDbCommand(sqlStr, conn);
